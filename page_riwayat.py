...
def page_about():
    ...
    if len(rows) > 0:
        ...
        st.download_button(label='Download Excel', data=excel_data, file_name=f'data_mahasiswa {today}.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')

        # Tombol untuk mengunduh file PDF dengan menambahkan tanggal unduhnya 
        st.download_button(label='Download PDF', data=pdf_data, file_name=f'Prediksi Kelulusan {today}.pdf', mime='application/pdf')
